...
import { verifyInbound } from '../src/feature/verifiers.js';
import { normalizeProviderEventId } from '../src/part/normalizer.js';
import { sanitizeObject, sanitizeText } from '../src/part/sanitize.js';
import { parseChronycOffset } from '../src/part/clock.js';
import { TgServerLogSink } from '../src/feature/tgserver-log.js';
import type { SourceConfig } from '../src/part/types.js';

function req(headers: Record<string, string>): Request { return { headers } as unknown as Request; }
function nowSeconds(): number { return Math.floor(Date.now() / 1000); }
const baseSource = (provider: SourceConfig['provider'], secretEnv: string): SourceConfig => ({ id: 's', appId: 'app', name: provider, slug: provider, provider, secretEnv, toleranceSeconds: 300, enabled: true });
...
describe('inbound verifiers', () => {
  ...
  it('rejects GitHub requests without a stable delivery id', () => {
    process.env.GITHUB_SECRET = 'secret';
    const body = Buffer.from(JSON.stringify({ hello: 'world' }));
    const sig = crypto.createHmac('sha256', 'secret').update(body).digest('hex');
    const result = verifyInbound(req({ 'x-hub-signature-256': `sha256=${sig}`, 'x-github-event': 'push' }), baseSource('github','GITHUB_SECRET'), body);
    expect(result.ok).toBe(false);
  });
  ...
  it('rejects Stripe events without a stable event id', () => {
    process.env.STRIPE_SECRET = 'whsec_test';
  ...
  it('rejects Slack events without a stable event id', () => {
    process.env.SLACK_SECRET = 'slack_secret';
    const body = Buffer.from(JSON.stringify({ type: 'event_callback', event: { type: 'message' } }));
    const ts = nowSeconds().toString();
    const base = `v0:${ts}:${body.toString('utf8')}`;
    const sig = crypto.createHmac('sha256', 'slack_secret').update(base).digest('hex');
    const result = verifyInbound(req({ 'x-slack-request-timestamp': ts, 'x-slack-signature': `v0=${sig}` }), baseSource('slack','SLACK_SECRET'), body);
  ...
  it('rejects Telegram updates without a stable update id', () => {
    process.env.TELEGRAM_SECRET = 'telegram_token';
    const body = Buffer.from(JSON.stringify({ message: { text: 'hi' } }));
    const result = verifyInbound(req({ 'x-telegram-bot-api-secret-token': 'telegram_token' }), baseSource('telegram','TELEGRAM_SECRET'), body);
    expect(result.ok).toBe(false);
  });
  ...
  it('rejects Generic HMAC events without a stable event id', () => {
    process.env.GENERIC_SECRET = 'generic_secret';
    const body = Buffer.from(JSON.stringify({ type: 'generic.created' }));
    const ts = nowSeconds().toString();
    const sig = crypto.createHmac('sha256', 'generic_secret').update(`${ts}.${body.toString('utf8')}`).digest('hex');
    const source: SourceConfig = { ...baseSource('generic-hmac-sha256','GENERIC_SECRET'), generic: { signatureHeader: 'x-signature', timestampHeader: 'x-timestamp', eventTypeHeader: 'x-event-type', signatureEncoding: 'hex', signaturePrefix: 'sha256=', signedContent: 'timestamp.body' } };
    const result = verifyInbound(req({ 'x-signature': `sha256=${sig}`, 'x-timestamp': ts, 'x-event-type': 'generic.created' }), source, body);
    expect(result.ok).toBe(false);
  });
});

describe('hardening helpers', () => {
  it('normalizes generic provider event ids only', () => { expect(normalizeProviderEventId('generic-hmac-sha256', ' ABC ')).toBe('abc'); expect(normalizeProviderEventId('github', ' ABC ')).toBe(' ABC '); });
  it('sanitizes sensitive fields and values', () => { expect(sanitizeObject({ token: 'abc', nested: { ok: 'yes' } })).toEqual({ token: '[REDACTED]', nested: { ok: 'yes' } }); expect(sanitizeText('Authorization: Bearer abc.def.ghi')).toContain('[REDACTED]'); expect(sanitizeText('secret=abc')).toBe('secret=[REDACTED]'); });
  it('parses chronyc offsets', () => { expect(parseChronycOffset('Last offset     : +0.012345 seconds')).toBeCloseTo(0.012345); });
  it('TGServer log sink does not throw when logging is disabled or unconfigured', () => { const sink = new TgServerLogSink(); expect(() => sink.log({ level: 'info', event: 'test', component: 'test', message: 'hello secret=abc' })).not.toThrow(); });
});
