...
app.get('/readyz', async (_req, res) => {
  ...
  let ok = true;
  try { await pool.query('SELECT 1'); checks.postgres = true; } catch (err: any) { ok = false; checks.postgres = String(err.message ?? err); }
  try { await redisConnection.ping(); checks.redis = true; } catch (err: any) { ok = false; checks.redis = String(err.message ?? err); }
...
app.get('/admin/events', requireAdmin, async (req, res) => {
  ...
  const rows = await pool.query(`SELECT id, source_id, provider, provider_event_id, event_type, status, received_at, updated_at FROM events ORDER BY received_at DESC LIMIT $1`, [limit]);
...
app.post('/admin/events/:id/replay', requireAdmin, async (req, res) => {
  ...
  const deliveries = await pool.query(`WITH picked AS (SELECT id FROM deliveries WHERE event_id=$1 AND status IN ('queued','retrying','unknown','dead') ORDER BY created_at ASC LIMIT $2) UPDATE deliveries d SET status='queued', next_attempt_at=NULL, updated_at=now() FROM picked WHERE d.id=picked.id RETURNING d.id`, [req.params.id, env.REPLAY_MAX_DELIVERIES_PER_REQUEST]);
  ...
  logGatewayEvent({ level: 'warn', event: 'admin_event_replay', component: 'api-system', message: 'Admin replay requested for event', eventId: req.params.id, details: { requeued: deliveries.rows.length } });
...
app.post('/admin/deliveries/:id/replay', requireAdmin, async (req, res) => {
  ...
  const delivery = await pool.query(`UPDATE deliveries SET status='queued', next_attempt_at=NULL, updated_at=now() WHERE id=$1 RETURNING id`, [req.params.id]);
  ...
  logGatewayEvent({ level: 'warn', event: 'admin_delivery_replay', component: 'api-system', message: 'Admin replay requested for delivery', deliveryId: req.params.id });
...
app.post('/ingress/:slug', express.raw({ type: '*/*', limit: env.MAX_BODY_BYTES }), async (req, res) => {
  ...
  if (!limit.allowed) {
    ...
    logGatewayEvent({ level: 'warn', event: 'ingress_rate_limited', component: 'api-system', message: 'Ingress request rate limited', sourceId: source.id, details: { provider: source.provider, retryAfterSeconds: limit.retryAfterSeconds } });
  ...
  if (!verified.ok) {
    ...
    logGatewayEvent({ level: 'warn', event: 'ingress_rejected', component: 'api-system', message: 'Ingress verification failed', sourceId: source.id, details: { provider: source.provider, reason: verified.reason, statusCode: verified.statusCode } });
...
async function replayDenied(req: express.Request, res: express.Response, type: 'event' | 'delivery', id: string, retryAfterSeconds: number): Promise<void> {
  ...
  logGatewayEvent({ level: 'warn', event: 'admin_replay_rate_limited', component: 'api-system', message: 'Admin replay was rate limited', details: { type, id, retryAfterSeconds } });
...
export async function startApiSystem(): Promise<void> {
  ...
  const server = app.listen(env.PORT, '0.0.0.0', () => {
    logGatewayEvent({ level: 'info', event: 'api_started', component: 'api-system', message: `webhook gateway api listening on ${env.PORT}` });
  });
  const shutdown = async () => {
    logGatewayEvent({ level: 'info', event: 'api_shutdown', component: 'api-system', message: 'webhook gateway api shutting down' });
    server.close();
